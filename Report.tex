...
\begin{document}
\title{A Novel Approach to Music Recommendation}
...
\maketitle
\begin{abstract}
Using digital signal processing techniques, novel approaches to the classification and comparison of music files are explored. The Fourier Transform is applied to analyze song frequency data, which is then segmented based on musical octave ranges. Using this segmented frequency data, songs are represented with a complex vector. From this complex vector, a method for quantifying the similarity between two songs is developed using the L2-Norm to find the distance between their vectors. From this distance, a list is generated of music recommendations based on a single song. In this study, the effectiveness of this approach is investigated through experimental procedures involving a collection of music files. The produced playlists are shown to be organized in logical orders based on the similarity of songs.
\end{abstract}
...
Music analysis often involves a commonly used function known as the Fourier Transform. A Fourier Transform is used to map time-domain audio information into a frequency-domain representation. Sound in the time domain is represented as a waveform, where amplitude is a function of time. This amplitude represents vibrations in the air, which the human ear interprets as sound. The frequency domain, however, represents sound as magnitude and phase as a function of frequency, which is more relevant than time information when analyzing music. For a computer to process this frequency signal, the amount of information present in the signal must be reduced through a process called \textit{sampling}. Sampling maps the continuous signal to a discrete representation; an array of values at (typically) evenly spaced indices of time. Stated here without derivation, the Discrete Fourier Transform, or DFT, is most commonly used to transform analog signals into digital frequency representations. See \cite{proakisDigitalSignalProcessing2007} for more detailed information.
...
\end{equation}
In \eqref{eq:DFT}, $N$ is the total number of samples in a time-domain sequence. $x(n)$ represents the value of that sequence at index $n$, and $k$ represents the frequency index for the Discrete Fourier Transform. Equation \eqref{eq:DFTMap} shows the DFT as a functional map from real to complex values. Each point within the DFT output represents a point in an N-dimensional complex vector space, where the magnitude and phase of that point represent the content of the time-domain signal at frequency $\frac{2k\pi}{N}$. These values depend on a concept known as \textit{sampling frequency}, which refers to the space between each sample of a signal in the frequency domain.
\begin{figure}[!htb]
...
In its original form, the DFT is computationally expensive and complex, comprised of $\mathcal{O}(N^{2})$ operations. An algorithm known as the Fast Fourier Transform (FFT) dramatically simplifies this complexity, reducing the expense to $\mathcal{O}(N*log(N))$ operations. It does this by leveraging symmetry within the DFT, decreasing the time necessary to complete this calculation. Equation \eqref{eq:DFT}, the basic representation for the DFT, is periodic about $N$, i.e. $X_{k+lN}=X_{k}$ for any integer $l$. The FFT uses this relationship to break the DFT into even and odd components, which is outlined in greater detail in \cite{hayesSchaumOutlineTheory1999}.
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \State $X\gets FFT(x)$
    \State $a=[\text{Array of Frequency Segment Breakpoints}]$
...
In order to use the frequency information generated by the FFT to compare two songs, it is important to generalize the information. Algorithm \eqref{alg:characterize} demonstrates a method for transforming the FFT output data into a generalized vector-based representation. The algorithm describes the process of taking the FFT of an audio file and \textit{segmenting} it. Segmenting the FFT data consists of seperating the data into frequency bands based on the values discussed in section \eqref{sec:AnalyzingMusic}. Within each band, the FFT bin (or index) with the highest magnitude will represent the most prominent frequency in that range. It is important to recognize that the FFT is normalized, which means that the indices will not correspond to individual frequencies, as shown in Fig.~\eqref{fig:FrequencyBins}. When segmenting the FFT, the frequency segments must be converted to an index value using \eqref{eq:BinNum}, where $N$ represents the number of samples in an FFT signal, $f$ representes the frequency value, and $F_{s}$ represents sampling rate. This formula uses these values to determine the bin into which a specific frequency will fall.
\begin{equation}\label{eq:BinNum}
  \text{Bin Index}=\frac{f * N}{F_{s}}
\end{equation}
This segmented FFT groups together several sequences of frequencies which make up similar ranges. Finding the bin with the largest magnitude within each segment produces an array of complex numbers, which presents a way to identify which frequencies are most prominent within a given audio file. Using the frequency segments as defined in section \eqref{sec:AnalyzingMusic} will produce a 5-element array, where each element is a complex number representing the most prominent frequency in a segment. The 5-element array stands as a characteristic representation of a single song, which can be used as a point of comparison. In order to determine the similarity between two songs, one must use these characteristic arrays and determine how similar those are. This can be accomplished using the L-2 Norm, \eqref{eq:L2Norm}, which provides a simple method for determining the physical distance between the endpoints of two vectors.
...
\begin{algorithm}
  ...
  \end{algorithmic}
\end{algorithm}
Algorithm~\eqref{alg:save_char} demonstrates the process for characterizing a directory with multiple .MP3 files and saving their characteristic arrays into a dictionary structure, which provides fast access and lookup time. Algorithm~\eqref{alg:save_comp} demonstrates the process for comparing 2 songs together and saving their comparison data. The comparison data produced by this function is a list of smaller 3-element lists, where the 3 elements are 'Song 1', 'Song 2', and the 'Distance' between them.
\begin{algorithm}
  ...
  \end{algorithmic}
\end{algorithm}
Using these two functions and the data structures that they create, a program creates a playlist from a filename taken as an argument. This program searches the list $C$ produced by Algorithm \eqref{alg:save_comp}, searching for any element that contains the input file, then creating a list sorted by distance. This sorted list can be shortened to produce a playlist of songs that are most similar to the input.
\section{Results}\label{sec:results}
Because of the qualitative nature of music, it is challenging to evaluate the system's actual performance. In order to examine how well the system performs, it is necessary to track the results produced using a relatively diverse collection of music files. Using a dataset of 50 songs, comprised of several songs each from 7 different artists, the system was consistently able to recommend logical sets of songs. One example output for this system is shown in Table~\eqref{tab:CompTab}, producing songs that are similar to 'Bridge Over Troubled Water', by Simon and Garfunkel.
...
\begin{table}[htbp]
    ...
    \end{tabular}
  \end{table}
This table displays song name and the 'Distance' metric, representing the similarity of the two songs. The top 10 songs are those that the program determined most similar to the input song. This result is consistent with design specifications, because those songs that are displayed at the top of the table are the same genre as the input, and many are written by the same artist. Conversely, the last rows in the table show the songs that the system found to be least similar, featuring songs with variation in instrumentation and melodic structure from the input song. The program consistantly produces similar results, generating playlists that are as similar to the initial song as the limited dataset allows. Performing the same operation on 'Breaking the Habit' by popular rock artist Linkin Park produces the output shown in Table~\eqref{tab:CompTab2}. Songs by other rock artists appear prevalent at the top of this table, while less similar songs appear toward the bottom. 
\begin{table}[htbp]
  \centering
...
% LocalWords:  DFT WAV FFT Spotify LastFM Tukey
